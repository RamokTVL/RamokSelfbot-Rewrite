...
{
    [Command("neko", "Send a NSFW image - NSFW")]
    class Neko : CommandBase
    {
            ...
            if (Message.Author.User.Id == Program.id)
            {
                if (JsonConvert.DeserializeObject<JSON>(System.IO.File.ReadAllText("config.json")).nsfw)
                {
                    EmbedMaker embed = new EmbedMaker()
                    {
                        ...
                        Description = "ooh",
                        ImageUrl = JsonConvert.DeserializeObject<NSFWOPTIONS>(new WebClient().DownloadString("https://nekobot.xyz/api/image?type=neko")).message
                    };
